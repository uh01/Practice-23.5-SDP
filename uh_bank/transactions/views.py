...
def send_transaction_email(user, amount, subject, template, target_account_number=None, source_account_number=None):
...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if amount > self.request.user.account.balance:
            return HttpResponseServerError('The bank is bankrupt. Unable to withdraw funds.')
...
class LoanRequestView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if current_loan_count >= 3:
            messages.error(self.request, "You have cross the loan limits")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if source_account.balance >= amount:
                ...
                send_transaction_email(request.user, amount, "Money Transfer Message", "transactions/money_transfer_email.html", target_account_number=target_account_number)

                receiver_user = get_user_model().objects.get(account=target_account)
                send_transaction_email(receiver_user, amount, "Money Received Message", "transactions/money_received_email.html", source_account_number=request.user.account.account_no)
